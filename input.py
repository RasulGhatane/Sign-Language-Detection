...
def save_labels(label):
    """
    Append new label to labels.txt if it doesn't exist.
    """
    # Read existing labels
    existing_labels = set()
    if os.path.exists(LABELS_FILE):
        with open(LABELS_FILE, 'r') as f:
            existing_labels = set(line.strip().lower() for line in f)
    
    # Add new label if it doesn't exist
    if label.lower() not in existing_labels:
        with open(LABELS_FILE, 'a') as f:
            f.write(f"{label.lower()}\n")

def get_all_labels():
    """
    Read all labels from labels.txt
    """
...
def mediapipe_detection(image, model):
    """
    Process the image through MediaPipe
    """
...
def collect_data_for_action(action, sequence_count):
    """
    Collect data for a specific action using webcam
    """
...
def train_model():
    ...
    try:
        ...
        num_classes = len(get_all_labels())
        ...
        logging.info("Model saved as 'keras_model.h5'")
...
def main():
    ...
    """
    while True:
        action = input("Enter 'collect' to collect data, 'train' to train the model, 'list' to see all labels, or 'q' to quit: ").lower()
        if action == 'q':
            break
        elif action == 'collect':
